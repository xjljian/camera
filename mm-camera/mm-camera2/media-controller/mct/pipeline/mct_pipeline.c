  ...
  unsigned int stream_id)
{
  ...
  if (!pipeline)
    return FALSE;
  int session_id = MCT_PIPELINE_SESSION(pipeline);
...
static boolean mct_pipeline_populate_query_cap_buffer(mct_pipeline_t *pipeline)
{
  ...
  boolean ret = TRUE;
  unsigned int i = 0, j = 0, common = FALSE;
  ...
  struct sysinfo info;
  uint32_t result;
  boolean enable_max_preview = FALSE;
...
#ifdef _ANDROID_
  property_get("persist.camera.max_prev.enable", str, "0");
  enable_max_preview = atoi(str);
  ...
  if(enable_max_preview) {
    uint32_t max_preview_width = 0;
    uint32_t max_preview_height = 0;
  ...
  uint8_t supported_effect_cnt = hal_data->supported_effects_cnt;
  ...
  memcpy(&hal_data->ubifocus_af_bracketing_need,
    &local_data->imaging_cap.ubifocus_af_bracketing,
  ...
  hal_data->min_required_pp_mask =
    local_data->pp_cap.min_required_pp_mask;
  ...
  mct_pipeline_t *pipeline)
{
  ...
  case CAM_PRIV_CANCEL_AUTO_FOCUS: {
    ...
    data->ret_value = *((int *)event_data.control_event_data);
  }
    break;
...
static void* mct_pipeline_stop_session_thread(void *data)
{
  ...
  if (module->stop_session) {
    module->stop_session(module, session_id);
  }
  pthread_mutex_lock(&thread_data->mutex);
  thread_data->started_num--;
  if(thread_data->started_num == 0)
    pthread_cond_signal(&thread_data->cond_v);
  pthread_mutex_unlock(&thread_data->mutex);
  return NULL;
}
...
void mct_pipeline_stop_session(mct_pipeline_t *pipeline)
{
 ...
 //stop tuning server
  ...
  timeToWait.tv_sec  += 1;
  rc = pthread_cond_timedwait(&pipeline->thread_data.cond_v,
