  ...
  ispif_port_t *ispif_sink_port, int num_streams, uint32_t *stream_ids)
{
  ...
  for (i = 0; i < num_streams; i++) {
    ...
    if (((ispif_sink_port->num_active_streams > 1) && (ispif_src_port->u.src_port.caps.use_pix) &&
       (!(ispif_sink_port->u.sink_port.sensor_cfg.fmt >= CAM_FORMAT_YUV_RAW_8BIT_YUYV &&
        ispif_sink_port->u.sink_port.sensor_cfg.fmt <= CAM_FORMAT_YUV_RAW_8BIT_VYUY))) ||
