 ...
 *  Return: nothing
 **/
static void module_sensor_offload_lens_reset(void* param1, void* param2,
 ...
 *  Return: nothing
 **/
static void module_sensor_offload_lens_move(void* param1, void* param2,
  void* param3, void* param4)
{
  af_update_t af_update;
  module_sensor_params_t *actuator_module_params;
  ...
  if (param1) {
    ...
    af_update.num_of_steps = (int)param3;
...
/* need to check whether to set the data loaded from kernel
  ...
  if (s_bundle->sensor_info->subdev_id[SUB_MODULE_ACTUATOR] != -1) {
    ...
    rc = module_sensor_params->func_tbl.process(
      module_sensor_params->sub_module_private,
      SENSOR_GET_ACTUATOR_NAME, (void*) &a_name);
    if (rc < 0) {
      SERR("failed rc %d", rc);
      goto ERROR2;
    }
 ...
 void *s_bundle, mct_event_t *event)
{
  ...
  if (bun->sensor_info->subdev_id[SUB_MODULE_ACTUATOR] != -1) {
    ...
    SERR("Error: No Actuator Module!");
    return FALSE;
  }
  actuator_module_params = bun->module_sensor_params[SUB_MODULE_ACTUATOR];
  lens_move_done_sig = &bun->af_bracket_params.af_bracket.lens_move_done_sig;

  ctrl = &(bun->af_bracket_params.ctrl);
  bracket = &(bun->af_bracket_params.af_bracket);
  ...
  if (ctrl->enable == TRUE) {
    if (ctrl->captured_count < num_of_burst) {
      ...
      if (!pthread_mutex_trylock(lens_move_done_sig)) {
        ...
        SLOW("moving lens, steps = %d", bracket->num_steps);
        /* Move Lens in Actuator */
        sensor_thread_msg_t msg;
        ...
        msg.param3 = (void*)bracket->num_steps;
        msg.param4 = &bracket->lens_move_done_sig;
        msg.stop_thread = FALSE;
        int nwrite = 0;
        SLOW("%s write fd %d", __func__, module_ctrl->pfd[1]);
        nwrite = write(module_ctrl->pfd[1], &msg, sizeof(sensor_thread_msg_t));
        if(nwrite < 0) {
          SERR("%s: Writing into fd failed",__func__);
        }
    ...
    SHIGH("AF Bracketing not enabled");
  }
  return TRUE;
}
  ...
  module_sensor_bundle_info_t * s_bundle, mct_event_control_parm_t *event_parm)
{
  ...
  bracket = &(s_bundle->af_bracket_params.af_bracket);
  ...
  ret = actuator_module_params->func_tbl.process(
    actuator_module_params->sub_module_private,
    ACTUATOR_GET_AF_ALGO_PARAM_PTR, &af_tune);
  if (ret < 0 || !af_tune) {
    SERR("failed to get af_tune rc %d af_tune_ptr %p", ret, af_tune);
    return FALSE;
  }

  /* Calculate bracket */
  af_algo = &af_tune->af_algo;
  total_steps = af_algo->position_far_end - af_algo->position_near_end + 1;
  total_steps += af_algo->undershoot_adjust;
  if (total_steps < bracket->burst_count) {
    SERR("Invalid params: total steps %d, burst %d",
      total_steps, bracket->burst_count);
    return FALSE;
  }
  bracket->num_steps = total_steps / bracket->burst_count;
  ...
  for (i = 0; i < res_cfg_table->size; i++) {
    ...
    case SENSOR_SET_START_STREAM: {
      ...
      if (rc < 0) {
        ...
        sensor_bracket_params_t *af_bracket_params = NULL;
        af_bracket_params = &(s_bundle->af_bracket_params);
        sensor_af_bracket_t * bracket = NULL;
        bracket = &(s_bundle->af_bracket_params.af_bracket);
        ...
        if (CAM_STREAM_TYPE_POSTVIEW == stream_info->stream_type) {
          if (af_bracket_params->af_bracket.enable == TRUE) {
            bracket->preview_identity = event->identity;
          }
        ...
        if (CAM_STREAM_TYPE_SNAPSHOT == stream_info->stream_type) {
          if (af_bracket_params->af_bracket.enable == TRUE) {
            bracket->snapshot_identity = event->identity;
          }
          if (flash_bracket_params->flash_bracket.enable == TRUE) {
        ...
        /* enable AOST feature upon streamon for both streams */
        if (bracket->preview_identity && bracket->snapshot_identity) {
          if (af_bracket_params->af_bracket.enable == TRUE) {
            af_update_t af_update;
            sensor_bracket_ctrl_t * ctrl = NULL;
            uint8_t num_of_burst;
            ...
            ctrl = &(af_bracket_params->ctrl);
            num_of_burst = bracket->burst_count;

            SHIGH("AF_bracketing stream on config");
            /* Reset lens for first bracketing position */
            if (s_bundle->sensor_info->subdev_id[SUB_MODULE_ACTUATOR] != -1) {
              actuator_module_params =
                s_bundle->module_sensor_params[SUB_MODULE_ACTUATOR];
              ...
              pthread_mutex_trylock(&bracket->lens_move_done_sig);
              ...
              SERR(" Error, no Actuator, cant reset lens");
            }

            /* fill internal params */
            af_bracket_params->ctrl.enable = TRUE;
            ctrl->min_frame_idx = ACTUATOR_MAX_WAIT_FRAME;
            ctrl->max_frame_idx = ctrl->min_frame_idx
              + (ACTUATOR_MAX_WAIT_FRAME + 1) * (num_of_burst - 1);

            /* post metadata msg to bus */
            bus_msg.sessionid = s_bundle->sensor_info->session_id;
            bus_msg.type = MCT_BUS_MSG_ZSL_TAKE_PICT_DONE;
            range.min_frame_idx = ctrl->min_frame_idx;
            range.max_frame_idx = ctrl->max_frame_idx;
            bus_msg.msg = &range;
            SHIGH(" Posting af_bracket msg to bus, sof %d, min:%d max:%d",
              s_bundle->last_idx, range.min_frame_idx, range.max_frame_idx);
            if(mct_module_post_bus_msg(module, &bus_msg) != TRUE) {
              SERR("Failed to send af_bracket msg");
            }
          } else {
            SLOW("AF_bracketing off, external enable %d",
              af_bracket_params->af_bracket.enable);
          }
        } else {
          SLOW("AF_bracketing off for non-snapshot");
        }
    ...
    case SENSOR_LOAD_CHROMATIX: {
      ...
      if (s_bundle->sensor_info->subdev_id[SUB_MODULE_ACTUATOR] != -1) {
        ...
        if (actuator_module_params->func_tbl.process) {
          ...
          rc = actuator_module_params->func_tbl.process(
            actuator_module_params->sub_module_private,
            ACTUATOR_GET_AF_ALGO_PARAM_PTR, &af_tune_ptr);
          if (rc < 0 || !af_tune_ptr) {
   ...
   sensor_bundle_info_t *bundle)
{
  ...
  case CAM_INTF_PARM_SET_RELOAD_AFTUNE: {
    ...
    void *algo_tune_ptr = (void *) ((size_t)driver_tune_ptr + sizeof(actuator_driver_params_t));

     memcpy(af_driver_ptr, driver_tune_ptr, sizeof(actuator_driver_params_t));
     memcpy(af_tune_ptr, algo_tune_ptr, sizeof(af_algo_tune_parms_t));
  ...
  switch (event_control->type) {
   ...
   case CAM_INTF_PARM_GET_AFTUNE: {
      ...
      if (s_bundle->sensor_info->subdev_id[SUB_MODULE_ACTUATOR] != -1) {
        ...
        memcpy(driver_tune_ptr, af_driver_ptr, sizeof(actuator_driver_params_t));
        memcpy(algo_tune_ptr, af_tune_ptr, sizeof(af_algo_tune_parms_t));
      }
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_CONTROL_STREAMOFF: {
    ...
    if (TRUE == module_sensor_is_ready_for_stream_off(module, event,
      ...
      sensor_af_bracket_t * bracket =
        &(bundle_info.s_bundle->af_bracket_params.af_bracket);
      sensor_bracket_params_t *flash_bracket_params =
        &(bundle_info.s_bundle->flash_bracket_params);

      if (bracket->preview_identity) {
        identity = &bracket->preview_identity;
      } else if (flash_bracket_params->flash_bracket.preview_identity) {
        identity = &flash_bracket_params->flash_bracket.preview_identity;
      }

      if (identity) {
        sensor_util_set_frame_skip_to_isp(module, *identity,
          bundle_info.s_bundle->hal_frame_skip_pattern);
        *identity = 0;
        identity = NULL;
      }

      if (bracket->snapshot_identity) {
        identity = &bracket->snapshot_identity;
      } else if (flash_bracket_params->flash_bracket.snapshot_identity) {
        identity = &flash_bracket_params->flash_bracket.snapshot_identity;
      }
  ...
  case MCT_EVENT_CONTROL_START_ZSL_SNAPSHOT: {
      ...
      sensor_bracket_params_t *af_bracket_params = NULL;
      af_bracket_params = &(bundle_info.s_bundle->af_bracket_params);
      if (af_bracket_params->af_bracket.enable == TRUE) {
        sensor_af_bracket_t * bracket = NULL;
        sensor_bracket_ctrl_t * ctrl = NULL;
        uint8_t num_of_burst;
        ...
        bracket = &(bundle_info.s_bundle->af_bracket_params.af_bracket);
        ctrl = &(af_bracket_params->ctrl);
        num_of_burst = bracket->burst_count;

        bracket->snapshot_identity =
          sensor_util_find_stream_identity_by_stream_type_and_session(
            module, CAM_STREAM_TYPE_SNAPSHOT,
            SENSOR_SESSIONID(event->identity));
        bracket->preview_identity = event->identity;
        ...
        /* fill internal params */
        ctrl->min_frame_idx = bundle_info.s_bundle->last_idx
          + ACTUATOR_MAX_WAIT_FRAME;
        ctrl->max_frame_idx = ctrl->min_frame_idx
          + (ACTUATOR_MAX_WAIT_FRAME + 1) * (num_of_burst - 1);
        af_bracket_params->ctrl.enable = TRUE;
...
static boolean module_sensor_free_bundle(void *data, void *user_data)
{
  ...
  lens_move_done_sig =
    &s_bundle->af_bracket_params.af_bracket.lens_move_done_sig;
    ...
    while (1) {
      ...
      if (entity.type == MEDIA_ENT_T_V4L2_SUBDEV &&
        ...
        *sensor_bundle->sensor_info = cfg.cfg.sensor_info;
        ...
        lens_move_done_sig =
          &sensor_bundle->af_bracket_params.af_bracket.lens_move_done_sig;
