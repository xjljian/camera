...
void eeprom_autofocus_calibration(void *e_ctrl) {
  ...
  if(0 != ectrl->eeprom_data.afc.starting_dac) {
    aftune->region_params[0].code_per_step =
      (ectrl->eeprom_data.afc.starting_dac /
      (aftune->region_params[0].step_bound[0] -
      aftune->region_params[0].step_bound[1]));
  }
  if (0 != ectrl->eeprom_data.afc.macro_dac &&
      0 != ectrl->eeprom_data.afc.infinity_dac) {
      aftune->region_params[1].code_per_step =
        ((ectrl->eeprom_data.afc.macro_dac -
        ectrl->eeprom_data.afc.infinity_dac) /
