 ...
 *
 *  @ptr: pointer to actuator_data_t struct
 *  @data: pointer to af_algo_tune_parms_t *
 *
 *  Return: 0 for success and negative error on failure
 *
 *  This function runs ringing test by moving lens from macro
...
static int actuator_get_af_algo_param_ptr(void *ptr, void *data)
{
  actuator_data_t *af_actuator_ptr = (actuator_data_t *)ptr;
  af_algo_tune_parms_t **af_algo_ptr = (af_algo_tune_parms_t **)data;
  if (!af_actuator_ptr || !af_algo_ptr) {
    SERR("failed af_actuator_ptr %p af_tune %p", af_actuator_ptr, af_algo_ptr);
    return -EINVAL;
  }
  *af_algo_ptr = af_actuator_ptr->ctrl->af_algo_ctrl;
  return 0;
}
...
static int actuator_get_af_driver_param_ptr(void *ptr, void *data)
{
  actuator_data_t *af_actuator_ptr = (actuator_data_t *)ptr;
