...
#define ROUND_INTERP(a,b,r,x) ({ \
  float a_f = (float) a; \
  float b_f = (float) b; \
  float res = LINEAR_INTERPOLATION(a_f, b_f, r); \
  x = round(res); \
})
...
{
  ROUND_INTERP(in1->c0, in2->c0, ratio, out->c0);
  ROUND_INTERP(in1->c1, in2->c1, ratio, out->c1);
  ROUND_INTERP(in1->c2, in2->c2, ratio, out->c2);
  ROUND_INTERP(in1->c3, in2->c3, ratio, out->c3);
  ROUND_INTERP(in1->c4, in2->c4, ratio, out->c4);
  ROUND_INTERP(in1->c5, in2->c5, ratio, out->c5);
  ROUND_INTERP(in1->c6, in2->c6, ratio, out->c6);
  ROUND_INTERP(in1->c7, in2->c7, ratio, out->c7);
  ROUND_INTERP(in1->c8, in2->c8, ratio, out->c8);

  ROUND_INTERP(in1->k0, in2->k0, ratio, out->k0);
  ROUND_INTERP(in1->k1, in2->k1, ratio, out->k1);
  ROUND_INTERP(in1->k2, in2->k2, ratio, out->k2);
  ...
  color_correct_type *tblOut, isp_pix_trigger_update_input_t *in_params)
{
  ...
  chromatix_CC_type *chromatix_CC = &chrPtr->chromatix_VFE.chromatix_CC;
  ...
  color_correct_type* tbl_out, isp_pix_trigger_update_input_t* in_params)
{
  ...
  /* required for finding CCT */
  trigger_info.mired_color_temp = MIRED(color_temp);
  ...
  switch (cct_type) {
    ...
    case AWB_CCT_TYPE_TL84_A:
       ratio = GET_INTERPOLATION_RATIO(1.0f / color_temp,
         1.0f / chromatix_CC->CC_A_trigger.CCT_start,
         1.0f / chromatix_CC->CC_A_trigger.CCT_end);
      util_color_correct_interpolate(
        &mod->table.chromatix_TL84_color_correction,
    ...
    case AWB_CCT_TYPE_D65_TL84:
      ratio = GET_INTERPOLATION_RATIO(1.0f / color_temp,
        1.0f / chromatix_CC->CC_Daylight_trigger.CCT_end,
        1.0f / chromatix_CC->CC_Daylight_trigger.CCT_start);
      util_color_correct_interpolate(
        &mod->table.chromatix_D65_color_correction_VF,
  ...
  isp_hw_pix_setting_params_t *in_params, uint32_t in_param_size)
{
  ...
  if(type & (1 << ISP_EFFECT_SATURATION)) {
    s = 2.0 * in_params->effects.saturation * mod->saturation_adjust;
    if (F_EQUAL(in_params->effects.hue, 0))
      util_color_correct_populate_matrix(mod->effects_matrix, s);
    CDBG_HIGH("%s: saturation level = %f, asd_saturae_adjust = %f\n",
      __func__, s, mod->saturation_adjust);
  }
...
/** color_correct_asd_adjust_saturation:
 *    @mod: CC module instance
 *    @in_params: input paramters
 *    @in_params_size: input paramters size
 *
 ...
 *          -1 - Parameters size mismatch
 **/
static int color_correct_asd_adjust_saturation(isp_color_correct_mod_t *mod,
  isp_pix_trigger_update_input_t *in_params, uint32_t in_param_size)
{
  float saturate_level = 1.0;;
  ...
  if (F_EQUAL(in_params->trigger_input.stats_update.asd_update.saturation_adjust, 0.0)) {
    CDBG_ERROR("%s: saturation adjust= 0, return!\n", __func__);
    return 0;
  }

  mod->saturation_adjust =
    in_params->trigger_input.stats_update.asd_update.saturation_adjust;

  /*decide base saturate level by current saturation effect*/
  if(in_params->cfg.effects.effect_type_mask & (1 << ISP_EFFECT_SATURATION)) {
    saturate_level = 2.0 * in_params->cfg.effects.saturation;
  } else {
    /*if no effect, then default level= 1*/
    saturate_level = 1.0;
  }

  SET_UNITY_MATRIX(mod->effects_matrix, 3);
  util_color_correct_populate_matrix(
    mod->effects_matrix, saturate_level * mod->saturation_adjust);

  CDBG_HIGH("%s: asd effect_saturate_level = %f, saturation adjust = %f\n",
    __func__, saturate_level, mod->saturation_adjust);
  ...
  isp_pix_trigger_update_input_t *in_params, uint32_t in_param_size)
{
  ...
  if (in_params->cfg.bestshot_mode != CAM_SCENE_MODE_OFF &&
    in_params->cfg.bestshot_mode != CAM_SCENE_MODE_AUTO) {
    SET_UNITY_MATRIX(effects_matrix, 3);
    p_effects_matrix = effects_matrix;
  } else if (in_params->cfg.bestshot_mode == CAM_SCENE_MODE_AUTO) {
    color_correct_asd_adjust_saturation(mod,
      in_params, in_param_size);
 ...
 *  This function runs in ISP HW thread context.
 *
 *  This function gets a parameter from CC module
 *
 *  Return:   0 - Success
 ...
 **/
isp_ops_t *color_correct40_open(uint32_t version)
{
