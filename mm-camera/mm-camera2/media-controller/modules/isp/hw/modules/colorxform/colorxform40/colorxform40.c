...
static int color_xform_do_hw_update(isp_color_xform_mod_t *mod)
{
  ...
  if (mod->hw_update_pending) {
    ...
    mod->applied_reg_cmd[ISP_COLOR_XFORM_ENC] =
      mod->reg_cmd[ISP_COLOR_XFORM_ENC];
    mod->applied_reg_cmd[ISP_COLOR_XFORM_VIEWFINDER] =
      mod->reg_cmd[ISP_COLOR_XFORM_VIEWFINDER];
    mod->hw_update_pending = 0;
  }
  return rc;
} /* color_xform_do_hw_update */
 ...
 **/
isp_ops_t *color_xform40_open(uint32_t version)
{
