...
int isp_hw_proc_hw_update(void *ctrl)
{
  ...
  isp_hw->dump_info.meta_data.lux_idx = FLOAT_TO_Q(10,
  ...
  struct msm_isp_event_data *sof)
{
  ...
  pending_update_params.session_id = isp_hw->pipeline.session_id[VFE_PIX_0];
  pending_update_params.frame_id = sof->frame_id;
  pending_update_params.dev_idx = isp_hw->init_params.dev_idx;
  ...
  case ISP_MSM8939: {
    *isp_version = SET_ISP_VERSION(ISP_VERSION_40, ISP_REVISION_V2);
    ...
    if (ver == ISP_MSM8916) {
      ...
      /*On 8916 limit ZSL+Longshot FPS to 4.5 at full resolution and
        ZSL only to 7.5. These values are arrived at by analysing BUS
        characteristics*/
