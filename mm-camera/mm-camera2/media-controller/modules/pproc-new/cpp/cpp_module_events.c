  ...
  unsigned int identity, isp_buf_divert_t *isp_buf)
{
  ...
  /* Step 2. if unprocessed divert is needed, add an event for that */
  ...
  mct_event_t* event)
{
  ...
  stream_params->hw_params.sensor_dim_info.width = sensor_out_info->dim_output.width;
  stream_params->hw_params.sensor_dim_info.height = sensor_out_info->dim_output.height;
