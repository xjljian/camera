  ...
  struct msm_cpp_frame_strip_info *out_ary)
{
  ...
  out_ary->compl_destination_address =
    in_ary->compl_destination_address +
    out_ary->dst_start_x * out_ary->bytes_per_pixel +
    out_ary->dst_start_y * in_ary->dst_stride;
}
  ...
  struct msm_cpp_frame_strip_info *out_ary)
{
  ...
  in_ary->source_address + out_ary->src_start_x * in_ary->bytes_per_pixel +
                         ...
                         int num_stripes)
{
  ...
  for (i = 0; i < num_stripes; i++) {
    ...
    if (in_info->output_plane_fmt == PLANE_Y ||
        in_info->output_plane_fmt == PLANE_CB ||
        in_info->output_plane_fmt == PLANE_CR) {
  ...
  int stripe_num)
{
  ...
  if (rot_info->rot_cfg < ROT_90) {
    ...
    we_info->buffer_ptr[2] = stripe_info->compl_destination_address;
...
int32_t cpp_params_calculate_crop(cpp_hardware_params_t *hw_params)
{
  ...
  CDBG_LOW("%s:%d, pwfp=%d, pwfl=%d, pww=%d, pwh=%d", __func__, __LINE__,
    hw_params->crop_info.process_window_first_pixel,
    hw_params->crop_info.process_window_first_line,
    hw_params->crop_info.process_window_width,
    hw_params->crop_info.process_window_height);
  ...
  cpp_hardware_params_t *hw_params)
{
  ...
  if (!hw_params->sensor_dim_info.width ||
      !hw_params->sensor_dim_info.height ||
      !hw_params->output_info.width || !hw_params->output_info.height) {
    CDBG_ERROR("%s:%d failed: process window %d %d output %d %d\n",
      __func__, __LINE__, hw_params->sensor_dim_info.width,
      hw_params->sensor_dim_info.height, hw_params->output_info.width,
  ...
  scalor_ratio =
    ((float)hw_params->sensor_dim_info.width *
    (float)hw_params->sensor_dim_info.height) /
    (hw_params->output_info.width * hw_params->output_info.height);
  ...
  struct cpp_frame_info_t *frame_info)
{
 ...
 /* Hack to equalize the snpashots in ZSL and NON-ZSl mode when flip is not 0 */
    ...
    memset(&plane_info[i], 0, sizeof(struct cpp_plane_info_t));
      ...
      if(i > 0) {
        ...
        if (hw_params->uv_upsample_enable) {
         ...
         plane_info[i].source_address =
           plane_info[i-1].src_stride * hw_params->input_info.scanline * i +
           hw_params->input_info.plane_offsets[i-1];
         plane_info[i].destination_address =
           plane_info[i-1].dst_stride *
           plane_info[0].maximum_dst_stripe_height * i +
         ...
         if((hw_params->output_info.plane_fmt == CPP_PARAM_PLANE_CRCB420) &&
           (hw_params->input_info.plane_fmt != CPP_PARAM_PLANE_CRCB420)){
             ...
             plane_info[i].compl_destination_address =
  ...
  CDBG_LOW("%s:%d] bf_enable:%d\n", __func__, __LINE__,
    hw_params->denoise_enable);
  return;
}
