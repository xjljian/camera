  ...
  unsigned int sessionid)
{
  ...
  pp_cap->min_required_pp_mask |= CAM_QCOM_FEATURE_CPP;
...
#else
  pp_cap->feature_mask |= (CAM_QCOM_FEATURE_CROP |
    CAM_QCOM_FEATURE_CPP | CAM_QCOM_FEATURE_FLIP | CAM_QCOM_FEATURE_SCALE);
#endif
  return TRUE;
}
      ...
      if(ctrl->session_params[i]->session_id == session_id) {
        for(j=0; j < CPP_MODULE_MAX_STREAMS; j++) {
          if(ctrl->session_params[i]->stream_params[j] == NULL) {
            ...
            /* Disable ASF if there is no Sharpness feature mask */
