...
static boolean pproc_port_add_modules_to_stream(
  ...
  mct_module_t *submod2, mct_module_t *submod3, mct_stream_info_t *stream_info,
  mct_port_t *port)
{
  boolean      rc = FALSE;
  ...
  if (submod2 && submod1) {
    ...
    if (submod3) {
      submod2->set_mod(submod2, MCT_MODULE_FLAG_INDEXABLE,
        stream_info->identity);
      submod3->set_mod(submod3, MCT_MODULE_FLAG_SINK,
        stream_info->identity);
    } else {
  ...
  case CAM_STREAM_TYPE_SNAPSHOT: {
    ...
    /* In ZSL snapshot, run C2D to convert from YUV422i to NV21 */
      if (stream_info->streaming_mode == CAM_STREAMING_MODE_CONTINUOUS) {
        rc = pproc_port_add_modules_to_stream(port_stream_info, c2d,
          NULL, NULL, stream_info, port);
      } else {
        rc = pproc_port_add_modules_to_stream(port_stream_info, submod2,
          submod1, NULL, stream_info, port);
