  ...
  int sub_evt_type, void *data)
{
  ...
  if (private->aec_update_flag == FALSE ||
    private->in_zsl_capture == TRUE||private->stream_type == CAM_STREAM_TYPE_SNAPSHOT) {
...
static void aec_port_save_update(mct_port_t *port, aec_output_data_t *output)
{
  ...
  if(output->stats_update.aec_update.frame_id != private->cur_sof_id) {
    ...
    aec_port_send_exif_debug_data(port, &output->stats_update);
    return;
  }
  MCT_OBJECT_LOCK(port);
...
static void aec_port_callback(aec_output_data_t *output, void *p)
{
  ...
  if (AEC_UPDATE == output->type ) {
    ...
    aec_port_send_exif_debug_data(port, &private->aec_update_data);
  }
  return;
}
  ...
  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_BUF_DIVERT: {
    ...
    aec_port_parse_RDI_stats_AE((uint32_t *)aec_stats->yuv_stats.q3a_aec_stats.SY,
  ...
  case MCT_EVENT_MODULE_SET_CHROMATIX_PTR: {
    ...
    if (aec_msg != NULL ) {
      ...
      aec_tuning_params->aec_subsampling_factor = ((private->algo_opt_mask & STATS_MASK_AEC) ?
