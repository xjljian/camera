  ...
  /* populate the stats_upate object to be sent out*/
    ...
    if (AWB_UPDATE == output->type) {
      ...
      if(output->frame_id != private->cur_sof_id) {
        ...
        awb_port_update_awb_info(port, &output->stats_update);
        awb_port_send_exif_debug_data(port, &output->stats_update);
        return;
      }
       MCT_OBJECT_LOCK(port);
      ...
      awb_port_update_awb_info(port, &(private->awb_update_data));
      awb_port_send_exif_debug_data(port, &(private->awb_update_data));
    }
  return;
}
                                                  ...
                                                  mct_event_t *event)
{
  ...
  case MCT_EVENT_MODULE_SET_CHROMATIX_PTR: {
    ...
    if (awb_msg != NULL ) {
      ...
      p_awb_tuning_params->awb_subsampling_factor =
