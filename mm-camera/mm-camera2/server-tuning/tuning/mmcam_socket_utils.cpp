...
bool Socket::accept(Socket &new_socket) const
{
    ...
    if (new_socket.m_sock <= 0) {
        MMCAM_LOGE("Socket accept error: %s", strerror(errno));
        return false;
    }
...
ServerSocket::ServerSocket(int port, int pipefd)
{
    bool rc = Socket::create();
    MMCAM_ASSERT(rc == true, "Could not create server socket: %s", strerror(errno));

    rc = Socket::bind(port);
    MMCAM_ASSERT(rc == true, "Could not bind to port: %s", strerror(errno));

    rc = Socket::listen();
    MMCAM_ASSERT(rc == true, "Could not listen to socket: %s", strerror(errno));
...
bool ServerSocket::recv(std::string &s, size_t size, bool &pipe_event)
{
    bool rc = false;
