...
bool CamAdapter::GetPreviewBuffer(uint8_t **buffer)
{
    //set flag for preview buffer copy
    Lock();
    m_pending_preview_request = true;
    UnLock();

    //wait till buffer is ready
    if ( DataWait() == true) {
        *buffer = m_preview_buffer;
        return true;
    } else {
        return false;
    }
...
bool CamAdapter::GetSnapshotBuffer(uint8_t **buffer)
{
    //set flag for snapshot buffer copy
    Lock();
    m_pending_snapshot_request = true;
    UnLock();
...
void CamAdapter::CopyPreviewImage(void *ptr)
{
    ...
    if (m_old_preview_size != size) {
        delete[] m_preview_buffer;
        m_preview_buffer = new uint8_t[size];
...
void CamAdapter::CopySnapshotImage(void *ptr)
{
    ...
    if ((int32_t)m_curr_snapshot_size != params->size) {
        delete[] m_snapshot_buffer;
    ...
    int read_len = read(file_fd, m_snapshot_buffer, params->size);
    MMCAM_ASSERT(read_len == params->size, "Snapshot file read returned less size than expected");

    close(file_fd);

    m_preview_image_width = params->dim.width;
    m_preview_image_height = params->dim.height;
}

void CamAdapter::EventNotify(uint32_t type, void *data)
{
    if (TryLock() == 0) {
        switch (type) {
